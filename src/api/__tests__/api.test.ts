...
describe.skip('getData Tests', () => {
  ...
  it('Should traverse and make further api calls on main results', async () => {
    expect.assertions(3);
    MockedRequest.mockResolvedValueOnce([{ apiUrl: '/api/vehicle_ftype.json' }, { apiUrl: '/api/vehicle_xj.json' }]);
  ...
  it('Should ignore vehicles without valid price during traversing', () => {
    MockedRequest.mockResolvedValueOnce([{ apiUrl: '/api/ftype.json' }, { apiUrl: '/api/xe.json' }, { apiUrl: '/api/xj.json' }]);
